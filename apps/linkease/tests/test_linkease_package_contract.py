...
class LinkEasePackageContractTest(unittest.TestCase):
    ...
    def test_legacy_luci_file_proxy_uses_shared_unix_socket_only(self):
        ...
        self.assertIn("rm -f /var/run/linkease.sock", entry_init)
        self.assertIn("append_primary_arg --localApi", entry_init)
        self.assertIn("append_primary_arg /var/run/linkease.sock", entry_init)
    ...
    def test_linkeasefull_is_dependent_full_runtime_package(self):
        ...
        self.assertIn("PKG_SOURCE_DATE:=3.0.20", makefile)
        self.assertIn("ARCH_HEXCODE=8664", makefile)
        self.assertIn("ARCH_HEXCODE=aa64", makefile)
        self.assertIn("PKG_SOURCE_VERSION:=$(ARCH_HEXCODE)", makefile)
        ...
        self.assertNotIn("linkease-desktop", makefile)
        self.assertIn("f1456c06e3fd5dcdf7fa728fdf102ab4169c0e8ed9308ddae8a9b9faee278dea", makefile)
        self.assertIn("d3c268aced92425289f968477b5f4d81e6295a10ad39f15773f5eba451f1b5f6", makefile)
        ...
        self.assertNotIn("$(PKG_BUILD_DIR)/bin/linkease $(1)/usr/sbin/linkease", makefile)
        self.assertNotIn("apptunnel-client", makefile)
        self.assertIn("PKGARCH:=all", makefile)

        self.assertIn("ENTRY_INIT=/etc/init.d/linkease-app-entry", compat_init)
        self.assertIn('"$ENTRY_INIT" restart', compat_init)
        self.assertIn('"active":"linkeasefull"', compat_init)
        self.assertNotIn("procd_open_instance", compat_init)

        self.assertIn("ENTRYD=/usr/sbin/linkease-app-entryd", entry_init)
        self.assertIn("GATEWAY=/usr/sbin/linkease-app-gateway", entry_init)
        self.assertIn("PRIMARY=/usr/bin/linkease-full", entry_init)
        self.assertIn("config_load linkease_app_entry", entry_init)
        self.assertIn("primary_enabled()", entry_init)
        self.assertIn('--primary "$PRIMARY"', entry_init)
        self.assertIn('--primary-enabled="$primary"', entry_init)
        self.assertIn('--fallback "$GATEWAY"', entry_init)
        self.assertIn("append_primary_arg 127.0.0.1", entry_init)
        self.assertIn("append_primary_arg 19290", entry_init)
        self.assertIn("append_primary_arg /apps/", entry_init)
        self.assertIn("append_primary_arg 0.0.0.0:8897", entry_init)
        self.assertIn("append_primary_arg /var/run/linkease.sock", entry_init)
        self.assertIn('"LINKEASE_AUTH_PROVIDER=openwrt"', entry_init)
        self.assertIn("prepare_linkeasefull_runtime()", entry_init)
        self.assertIn("ensure_linkease_config()", entry_init)
        self.assertIn('uci -q get linkease.@linkease[0].local_home', entry_init)
        ...
        self.assertIn('conflict = false', controller)
        self.assertIn('conflict_service = ""', controller)
        self.assertNotIn('local linkease_enabled', controller)
        self.assertNotIn('uci:get_first("linkease", "linkease", "enabled")', controller)
        ...
        self.assertIn('http.getcookie(key)', auth_model)
        self.assertIn('"sysauth_https", "sysauth_http", "sysauth"', auth_model)
        self.assertIn('self.dependencies.ubus("session", "get", { ubus_rpc_session = sid })', auth_model)
        self.assertIn('"linkease_openwrt_sid=" .. sid .. "; Path=/apps; HttpOnly; SameSite=Lax"', auth_model)
        self.assertIn("valid_apps_return(value)", auth_model)
        self.assertIn('path == "/apps"', auth_model)
        self.assertIn('prefix == "/apps/" or prefix == "/apps?" or prefix == "/apps#"', auth_model)
        self.assertIn('value:match("^(https?://)([^/]+)(/.*)$")', auth_model)
        self.assertIn("authority_host(authority)", auth_model)
        self.assertIn("authority_host(self:request_authority())", auth_model)
        self.assertNotIn('request_host .. ":19290"', auth_model)
        self.assertNotIn('lan_host .. ":19290"', auth_model)
        self.assertIn("valid_cookie_value(sid)", auth_model)
        self.assertIn('local pending_return_cookie = "linkease_openwrt_pending_return"', auth_model)
        self.assertIn('pending_return_cookie .. "=" .. cookie_encode(target)', auth_model)
        self.assertIn('self.dependencies.build_url("admin", "services", "linkease_auth", "auth_finish", state)', auth_model)
        self.assertIn('function Bridge:auth_finish(state)', auth_model)
        self.assertIn('target = self:pending_return_target()', auth_model)
        ...
        self.assertNotIn("Please disable LinkEase before starting LinkEase Full", status)
        ...
        self.assertIn("/etc/init.d/linkeasefull disable", defaults)
        self.assertIn("/etc/init.d/linkease-app-entry enable", defaults)
        self.assertIn("/etc/init.d/linkease-app-entry restart", defaults)
