...
class LinkEasePackageContractTest(unittest.TestCase):
    ...
    def test_standard_linkease_uses_legacy_runtime_only(self):
        ...
        self.assertIn("/etc/init.d/linkeasefull stop", init)
        self.assertIn('uci -q set linkeasefull.@linkeasefull[0].enabled="0"', init)
        self.assertIn("stop_linkeaselite_runtime", init.split("procd_open_instance")[0])
        self.assertIn("stop_linkeasefull_runtime", init.split("procd_open_instance")[0])
