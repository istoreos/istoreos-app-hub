...
- ddnsto — DDNSTO路由远程 — DDNSTO内网穿透，可以外网远程你的路由器
- demon — 容器魔王 — 【魔王现世】为 iStoreOS 特制，收益更高。每月至高可赚「千元」，现在上线秒领「30天20%收益加成」，挂机托管自动赚米，拿到手软！替代网心云的「容器魔方」。
- dockermanager — Docker管理 — 更简单专业的 Docker 管理器，加速更快更方便.
- dpanel — DPanel 可视化面板 — 轻量化 docker 可视化管理面板。
- drawio — DrawIO绘图 — DrawIO是运行在浏览器中的在线绘图工具。
- emby — Emby影院 — Emby是一个多媒体串流平台，可以让您方便地管理和串流您的媒体。
- excalidraw — Excalidraw画板 — Excalidraw是支持多人协同的私有化在线画板工具。
- fastnet — FastNet 网络测速 — FastNet 提供网络测速与网络诊断 Web UI，默认端口 3200。
...
- istoredup — iStoreDup — iStoreOS 容器版。此插件已废弃，iStoreOS 请使用更方便的「ShadoWRT」，保留此插件仅为其他系统体验 iStoreOS 之用
- istoreenhance — KSpeeder(原iStore增强) — KSpeeder 为 Docker 镜像、软件包/文件下载和 GitHub/GitLab 公开仓库克隆提供网络加速。
- istorepanel — 1Panel — 1Panel 是新一代的 Linux 服务器运维管理面板
- istorerouter — iStoreRouter — 为 iStoreOS 提供路由状态、网络配置、无线设置、DDNS、存储管理和应用商店入口。
- istorex — iStoreNAS(原iStoreX) — 把 iStoreOS 一键变成 NAS 系统，网盘，同步，组网，时光机，AI 等一网打尽。
...
- mymind — MyMind思维导图 — MyMind 是一个在线思维导图编辑器。
- navidrome — Navidrome音乐 — Navidrome是开源音乐服务器，自建云端音乐播放器。
...
- photoprism — PhotoPrism相册管理 — PhotoPrism是一个AI驱动的相册管理应用。
- plex — Plex影院 — Plex是一个多媒体串流平台，可以让您方便地管理和串流您的媒体。
- pve — Proxmox虚拟机(PVE) — 运行在 iStoreOS 上面的 Proxmox(PVE) 虚拟机平台。
- rtbwmon — 实时流量 — 实时查看客户端网速，无后台服务
- runmynas — RunMyNAS自定义固件 — RunMyNAS可以让你根据自己的需要，自定义自己的 iStoreOS 系统。
- shadowrt — ShadoWRT — ShadoWRT（系统分身）可创建管理 iStoreOS/OpenWRT 的分身，分身可跟随本体升级且保留数据
- sunpanel — SunPanel导航页 — 一个易用好看的NAS导航页。
- systools — 系统便利工具 — 一些便利的工具集，方便修复一些问题，一键开关 IPv6。
- typecho — TypeCho博客 — TypeCho 是一个轻量级博客。
- ubuntu — Ubuntu — 带Web远程桌面的高性能Docker版Ubuntu。
- unifi — Unifi控制器 — Ubnt AC 控制器。
- uptimekuma — UptimeKuma — Uptime Kuma 是一款易于使用的自托管监控工具。
- vaultwarden — Vaultwarden私人密码箱 — Vaultwarden是Rust开发的Bitwarden服务器(私人密码箱)，包括Web Vault。
- wan-mac — MAC地址生成器 — 方便地修改WAN口MAC地址，可指定前缀随机生成MAC地址.（只支持 OpenWRT 21 以上）
- webvirtcloud — KVM虚拟机(WebVirtCloud) — iStoreOS上面的 KVM 虚拟机，基于 WebVirtCloud 实现。
- xteve — Xteve — Xteve是一个在线电视源的代理工具。
- xunlei — 迅雷下载 — 迅雷是一个常用的下载工具。本插件可部署 cnk3x 封装的 Docker 版迅雷。
- zdinnav — 智淀导航 — 智淀导航：网页书签管理工具，搭建专属于你的导航网站。
