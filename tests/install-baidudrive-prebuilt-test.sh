...
printf '%s\n' "$output" | grep -F 'opt/baidunas-glibc/ld-linux-x86-64.so.2' >/dev/null
...
grep -F 'BAIDU_NAS_GLIBC_DIR="/opt/baidunas-glibc"' "$ROOT_DIR/apps/baidudrive/baidudrive/files/baidudrive.init" >/dev/null

mkdir -p \
  "$TMP_DIR/split-app/baidudrive-binary-test" \
  "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/nas_sdk" \
  "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/glibc"

printf '#!/bin/sh\n' > "$TMP_DIR/split-app/baidudrive-binary-test/baidudrive.x86_64"
printf '#!/bin/sh\n' > "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/nas_sdk/baiduNas"
printf '#!/bin/sh\n' > "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/nas_sdk/P2PClient.bin"
printf 'kernel\n' > "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/nas_sdk/libkernel.so"
printf 'loader\n' > "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/glibc/ld-linux-x86-64.so.2"
printf 'libc\n' > "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/glibc/libc.so.6"
chmod +x \
  "$TMP_DIR/split-app/baidudrive-binary-test/baidudrive.x86_64" \
  "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/nas_sdk/baiduNas" \
  "$TMP_DIR/split-sdk/baidusdk-binary-test/x86_64/usr/lib/baidusdk/nas_sdk/P2PClient.bin"

tar -czf "$TMP_DIR/split-app/baidudrive-binary-test.tar.gz" -C "$TMP_DIR/split-app" baidudrive-binary-test
tar -czf "$TMP_DIR/split-sdk/baidusdk-binary-test.tar.gz" -C "$TMP_DIR/split-sdk" baidusdk-binary-test

split_output=$(
  BAIDUDRIVE_VERSION=test \
  BAIDUDRIVE_ARCH=x86_64 \
  BAIDUDRIVE_TARBALL_URL="file://$TMP_DIR/split-app/baidudrive-binary-test.tar.gz" \
  BAIDUDRIVE_SDK_TARBALL_URL="file://$TMP_DIR/split-sdk/baidusdk-binary-test.tar.gz" \
  "$ROOT_DIR/tools/install-baidudrive-prebuilt.sh" --dry-run
)

printf '%s\n' "$split_output" | grep -F 'usr/sbin/baidudrive' >/dev/null
printf '%s\n' "$split_output" | grep -F 'opt/baidunas-sdk/baiduNas' >/dev/null
printf '%s\n' "$split_output" | grep -F 'opt/baidunas-sdk/P2PClient.bin' >/dev/null
printf '%s\n' "$split_output" | grep -F 'opt/baidunas-glibc/ld-linux-x86-64.so.2' >/dev/null
printf '%s\n' "$split_output" | grep -F 'opt/baidunas-glibc/lib64/ld-linux-x86-64.so.2' >/dev/null
